...
public class DateOperations {
	...
	public static void printCurrentTime(String zoneSubstring) {
		for(String curZone: ZoneId.getAvailableZoneIds()) {
			if(curZone.contains(zoneSubstring)) {
				displayTime(curZone);
				return;
			}
//		
		}
		System.out.println("sorry, zone not found..");
	
	}
	
	private static void displayTime(String curZone) {
		
		LocalTime time = LocalTime.now(ZoneId.of(curZone));
		System.out.println("zone name containing " + curZone + " - " + time.format(hm));
	}
